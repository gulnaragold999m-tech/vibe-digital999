...
svezhee() {  # $1 — имя файла; печатает строку предупреждения или ничего
  ...
  # И не тревожим, если та правка уже вошла в нашу историю: тогда наша
  # версия новее, просто отличается. Иначе хук ругался бы на собственную
  # свежую работу — поймано на себе 13.08.2026.
  local bestc
  bestc=$(git log -1 --format='%H' "$best_ref" -- "$file" 2>/dev/null || true)
  [ -n "$bestc" ] && git merge-base --is-ancestor "$bestc" HEAD 2>/dev/null && return 0
  local kogda
...
3. ${LIVE}
   13.08.2026 ветки сведены: main и живая совпадают. Свою ветку влей
   в живую, когда закончишь, — иначе снова разъедется.
...
8. ГРАНАТ — НЕ КЛИЕНТ, а вторая организация владелицы. Оба сайта её,
   обе по одному адресу (уточнено 15.08.2026; до этого в правилах
   стояло «там владелица работает», и её же контакты в политике
   Граната приняли за чужую копипасту). Следствия: контакты Оператора
   на обоих сайтах её и это верно; две карточки в Яндекс Бизнесе
   в одном городе могут конкурировать друг с другом.
