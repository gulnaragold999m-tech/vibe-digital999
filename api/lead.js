 ...
 *
 * ВАЖНО ПРО ЯНДЕКС БИЗНЕС. Мы считали, что он меток не ставит: там
 * автоматическая реклама от карточки организации, а не кампании
 * с настраиваемыми ссылками. Отчёт Метрики за 6–9 августа показал
 * обратное — 376 из 441 визита пришли на адрес вида
 *
 *     vibe-digital999.ru/?utm_referer=geoadv_...
 *
 * Метка называется utm_referer, а не utm_source, и в стандартный набор
 * не входит. Первая версия этой функции её не читала — то есть в каждой
 * заявке с рекламы в строке «Реклама» стоял бы прочерк, хотя реклама
 * и есть источник восьмидесяти пяти процентов трафика.
 *
 * Теперь читаем и её. Порядок перебора от точного к общему: если вдруг
 * появятся настоящие utm_source и utm_campaign, они и попадут в заявку,
 * а geoadv останется запасным. */
function utmFrom(pageUrl) {
  ...
  /* Реклама Яндекс Бизнеса. Значение длинное и машинное — обрезаем,
     чтобы не разносить строку заявки на пол-экрана: для понимания
     «пришёл с рекламы Карт» хватает начала. */
  const geo = p.get('utm_referer') || p.get('yclid') || '';
  if (!geo) return '';
  return geo.startsWith('geoadv')
    ? 'Яндекс Бизнес (' + geo.slice(0, 40) + ')'
    : geo.slice(0, 60);
}
...
async function handleLead(req, res) {
  ...
  /* Город человек пишет сам, и это самый надёжный ответ на вопрос,
     какой регион приносит заявки.

     Реклама Яндекс Бизнеса метку всё-таки ставит — utm_referer=geoadv_…,
     см. utmFrom выше, — но она говорит только «пришёл с карточки»
     и ничего не говорит о городе: карточка одна на три региона.
     Поле необязательное: пустое оно ничего не ломает. */
