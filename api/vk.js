...
/**
 * Снимаем разметку Markdown перед отправкой.
 *
 * ВКонтакте её не понимает и показывает как есть. 12.08.2026 клиент
 * увидел в чате строку со звёздочками:
 *
 *   **Где сейчас чаще всего приходят клиенты — через WhatsApp, звонок…**
 *
 * Модель размечает жирным сам вопрос, потому что так её учили отвечать
 * в чатах, которые Markdown понимают. Запрещать ей это в промпте
 * ненадёжно — забудет через десять реплик. Надёжнее вычистить здесь,
 * на выходе: тогда правило выполняется всегда и без её участия.
 *
 * Заголовки и списки со звёздочкой переводим в обычные строки, а не
 * выбрасываем: перечисление в ответе бота осмысленное, теряется только
 * оформление.
 */
...
async function sendVk(peerId, text) {
  ...
  try {
    const result = await vkApi('messages.send', {
      ...
      message: stripMarkdown(text).slice(0, 4000),
    });
