...
/* Шрифты сайта, вшитые в разметку. Вынесены в marketing/shrifty.js —
   этим же кодом пользуется karty-cards.js, а копия кода в двух файлах
   и есть тот второй источник правды, из-за которого в проекте
   разъезжались цены. Почему вшиваем и почему Cinzel только
   латинский — там же, в комментарии. */
  ...
  chat: `
    <div class="mock-win">
      ...
      <div class="mock-body chat">
        ...
        <div class="bub out">Бот со сценарием — от 15 000 ₽, срок от 7 дней.
...
/* СРОКИ сверяем так же, и по той же причине.

   13.08.2026 владелица прислала свой рекламный макет рядом со снимком
   сайта на телефоне. Цена совпадала — сверка выше работала, — а срок
   нет: на макете «отчёт за 3–5 дней», на странице аудита «3 дня».
   И так во всех четырёх: «запуск от 3 дней» против «от 7 дней»,
   «срок 2–3 дня» против «от 7 дней», «от 2 недель» против «от 14 дней».

   Проверялись только цены, поэтому сроки разъехались молча и жили
   в рекламе неделю. Срок — такое же обещание, как цена, и нарушенный
   срок заметнее: цену обсуждают, а дату ждут.

   Берём срок из блока «Срок» на странице услуги и сравниваем ЧИСЛА:
   слова вокруг у макета свои («запуск от», «отчёт за»), и это нормально. */
const nums = (s) => (s.match(/\d+/g) || []).join('-');

const TERMS = Object.fromEntries(
  Object.entries(CHECK).map(([id, url]) => {
    const page = require(path.join(ROOT, 'src/pages.js')).PAGES.find(p => p.url === url);
    const html = page && page.file
      ? fs.readFileSync(path.join(ROOT, 'src/pages', page.file), 'utf8')
      : '';
    const m = html.match(/Срок<\/span>\s*<span class="page-fact-value">([^<]+)</);
    let znachenie = m ? m[1].trim() : '';

    /* С 13.08.2026 срок на странице стоит не текстом, а подстановкой
       {{срок:id}} — её разворачивает сборка из того же src/prices.js.
       Здесь читается ИСХОДНИК страницы, поэтому в руки попадал сам
       шаблон, а не срок: сверка не совпадала ни на одном макете,
       генератор выходил с ошибкой и не рисовал ничего.

       Поломка прожила незамеченной сутки ровно потому, что макеты
       после неё ни разу не перерисовывали. Проверка, которая валит
       сборку на ровном месте, ничем не лучше отсутствующей.

       Разворачиваем шаблон тем же прайсом, что и сборка. */
    const shablon = znachenie.match(/^\{\{срок:([a-zA-Z0-9-]+)\}\}$/);
    if (shablon) znachenie = PRICES.srok(shablon[1]);
...
for (const it of ITEMS) {
  ...
  const wantTerm = TERMS[it.id];
  if (wantTerm && nums(wantTerm) !== nums(it.term)) {
    console.error(`  ✗ ${it.id}: на макете «${it.term}», на сайте «${wantTerm}» (${CHECK[it.id]})`);
    mismatch++;
  }
}
if (mismatch) {
  console.error('\nМакеты разошлись с сайтом. Правьте ITEMS или src/pages — но так, чтобы совпало.\n');
  process.exit(1);
}
console.log('  цены и сроки сверены с сайтом: совпадают\n');
